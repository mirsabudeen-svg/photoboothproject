...
create table if not exists public.assistant_audit (
  ...
  user_id text not null,                -- Supabase auth user id ('dev-local' in unconfigured dev)
  tool varchar(64) not null,
  args jsonb not null default '{}',
  status varchar(16) not null,          -- executed_read | failed  (Phase B adds proposed/approved/denied/executed)
  result_summary text,
...
-- Authenticated dashboard users may READ the log (for the /settings activity view).
create policy "assistant_audit_read_authenticated"
  on public.assistant_audit for select
  to authenticated
  using (true);

-- No insert/update/delete policies: only the service-role key (server) can write.
